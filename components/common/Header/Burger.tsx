      ...
      <nav className="w-93.75 h-100svh min-h-full flex flex-col justify-between text-neutral-bright-beginning text-[14px] font-normal overflow-y-scroll">
        <ul className="flex flex-col text-neutral-bright-beginning text-[14px] font-nav font-normal">
          <li className="px-12 py-4">
            <Link className="uppercase" href="/about" onClick={toggleBurger}>{t("header.nav.1")}</Link>
          </li>
          <li className="px-12 py-4">
            <Link className="uppercase" href="/products" onClick={toggleBurger}>{t("header.nav.2")}</Link>
          </li>
          <li className="px-12 py-4">
            <Link className="uppercase" href="/expertise" onClick={toggleBurger}>{t("header.nav.3")}</Link>
          </li>
          <li className="px-12 py-4">
            <Link className="uppercase" href="/investors" onClick={toggleBurger}>{t("header.nav.4")}</Link>
          </li>
          <li className="px-12 py-4">
            <Link className="uppercase" href="/career" onClick={toggleBurger}>{t("header.nav.5")}</Link>
          </li>
          <li className="px-12 py-4">
            <Link className="uppercase" href="/contacts" onClick={toggleBurger}>{t("header.nav.6")}</Link>
          </li>
          <li className="px-12 py-4">
            <Link className="uppercase" href={`${locale === 'ru' ? '../en/' + path.slice(path.search("/\//g") + 5) : '../ru/' + path.slice(path.search("/\//g") + 5)}`} onClick={toggleBurger}>{t("header.nav.lang_burger")}</Link>
          </li>  
        </ul>
        <button
